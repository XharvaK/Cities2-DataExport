...
public sealed class TransitPerformanceBundleTests
{
    ...
    [Fact]
    public void TransitPerformanceBundle_NoTransitLines_IsOk()
    {
        var collector = new MetricsCollector(new NoTransitMetricProbe());
        ...
        Assert.Equal(MetricStatus.Ok, snapshot.Mobility.Status);
        Assert.Equal(0, snapshot.Mobility.LinesTotal);
        Assert.Equal(MetricStatus.Ok, snapshot.TransitPerformanceSemantics.Status);
        Assert.Equal(MetricStatus.Ok, snapshot.Meta.MetricStatus["transit_performance_semantics"]);
    }

    private sealed class NoTransitMetricProbe : IMetricProbe
    {
        ...
        public MobilitySummary CollectMobilitySummary() => new()
        {
            ...
            LinesTotal = 0,
            Lines = Array.Empty<MobilityLineRecord>(),
            TrafficVolumeIndex = 12.5
        };
    }
